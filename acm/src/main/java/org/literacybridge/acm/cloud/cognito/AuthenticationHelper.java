...
//        CognitoUserPool userPool = new CognitoUserPool(context, Constants.COGNITO_USER_POOL_ID, Constants.COGNITO_APP_CLIENT_ID, Constants.COGNITO_APP_SECRET, Constants.COGNITO_REGION);
    ...
    AuthenticationResult PerformSRPAuthentication(String username, String password) {
        ...
        try {
            ...
            if (ChallengeNameType.PASSWORD_VERIFIER.toString().equals(initiateAuthResult.getChallengeName())) {
                RespondToAuthChallengeRequest challengeRequest = userSrpAuthRequest(
                    initiateAuthResult,
                    password);
